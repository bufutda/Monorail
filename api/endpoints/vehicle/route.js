...
module.exports.hand = function (request, response, url, endpoint) {
    if (Endpoint.requireParam(url, ["type"], {type: ["bus", "train"]})) {
        if (url.query.type === "bus") {
            db.query("SELECT R.ID, R.rStart, R.rEnd, R.Num_stops FROM (Bus NATURAL JOIN Vehicle) AS V JOIN Route AS R ON R.ID = V.rID;", function (err, rows, feilds) {
                if (err) {
                    db.e.emit("error", err);
                } else {
                    endpoint.data = rows;
            ...
            db.query("SELECT R.ID, R.rStart, R.rEnd, R.Num_stops FROM (Train NATURAL JOIN Vehicle) AS V JOIN Route AS R ON R.ID = V.rID;", function (err, rows, feilds) {
                if (err) {
                    db.e.emit("error", err);
                } else {
                    endpoint.data = rows;
