...
module.exports.hand = function (request, response, url, endpoint) {
    if (Endpoint.requireParam(url, "id")) {
        if (utils.isInt(url.query.id)) {
            db.query("SELECT S.No, S.Location, S.rID FROM (mrdb.Stop AS S JOIN mrdb.Route AS R ON S.No = R.rStart) WHERE R.ID = " + db.escape(url.query.id) + ";", function (err, rows, fields) {
                if (err) {
                    db.e.emit("error", err);
                    return;
                }
...
module.exports.desc = "Get the first stop in a route";
